...
final class Psr4AutoloadMap
{
    ...
    {
        $currentNamespace = new PhpNamespace($directory->name());
        $currentDirectory = $directory;

        while (true) {
            foreach ($this->items as $item) {
                if ($item->directory()->equals($currentDirectory)) {
                    return $currentNamespace->removeFirstPart()->preprendNamespace($item->namespace());
                }
            }
            ...
            $currentNamespace = $currentNamespace->prepend($parent->name());
